...
const PackingCart = ({ loading }) => {
    ...
    const afterPurchase = async (payload) => {
        ...
        const getResponse = await initApi.data;
        if (getResponse.status == 1) dispatch(emptyCart()) //setCart([])
        // After getting response
        setProcessStatus(getResponse)
        setCart([])
    ...
    return (
        <>
            ...
            {/* <!-- Breadcrumb end --> */}
            {/* <!-- Table area start --> */}
            {processStatus.status == 1 && processStatus.status ?
                <section className="cart animate__animated animate__slideInUp p-0">
                    <div className="container my-4">
                        <RequestComplete quoteStatus={processStatus} redirect='/packing-materials' history={router} />
                    </div>
                </section>
                :
